...
#FILENAME = "../project_vscode/data/PEPE/2025/02/spammers/AIMemeIntern_202502.json" # 選擇要對哪個檔案執行
# "../project_vscode/data/DOGE/2021/4/DOGE_20210428_Latest654.json" 
#
ANALYSIS_NAME = "PEPE_AIMemeIntern_20250202_test"  # 設定要存的 txt 名稱
...
# 📁 路徑設定
INPUT_FOLDER = f"../project_vscode/data/{JSON_DICT_NAME}/{year}/{month}/spammers"
OUTPUT_FOLDER = f"../LCS/analysis/spammer_analysis/{JSON_DICT_NAME}/{year}/{month}"
...
if __name__ == "__main__":
    ...
    for filename in sorted(json_files):
        ...
        for tweet in tweets:
            author_counts[tweet["username"]] += 1

        if any(count > 1000 for count in author_counts.values()):
            print(f"⛔ 跳過 {filename}，因為有作者貼文數超過 1000")
            continue    
        ...
        if IS_CLUSTERED:
            ...
            authors = set(tweet["username"] for tweet in tweets)

            for author in authors:
                user_tweets = [tweet for tweet in tweets if tweet["username"] == author]
                if len(user_tweets) < 2:
                    continue

                writed_compare = 0 # 實際寫入的結果數
                safe_author = author.replace("/", "₿")  # 避免特殊字元造成檔名錯誤

                os.makedirs(OUTPUT_FOLDER, exist_ok=True)
                txtname = os.path.join(OUTPUT_FOLDER, f"{JSON_DICT_NAME}_{year}{month}_{safe_author}.txt")
                ...
                try:
                    with open(txtname, 'w', encoding="utf-8-sig") as filetxt:
                        with Pool(processes=cpu_count()) as pool:  # Pool(processes=cpu_count()): 建立一個「工作池」來開啟多個 CPU 核心跑比對
                            ...
                            # pool.imap_unordered(compare_pair, pairs): 將 pairs 中的每組 (i, j, tweets) 傳進 compare_pair 函式處理，並行處理，誰先完成就先送回來
                            for res in tqdm(pool.imap_unordered(compare_pair, generate_pairs(user_tweets)),
                                            total=(len(user_tweets) * (len(user_tweets) - 1)) // 2,  # 產生所有從 n 個元素中選出 2 個不重複且無順序的組合  [C(n, 2) = (n(n - 1)) / 2]
                                            desc=f"{author} 比對中"):  # tqdm(...): 進度條
                ...
                print(f"實際寫入的結果數：{writed_compare}")
                print(f"寫入比例：{writed_compare / len(tweets) * 100:.6f}%")
                print(f"✅ 已輸出結果到 {txtname}")
                print()
