...
def load_data():
    ...
    TWEET_PATH = f'../data/filtered_tweets/{COIN_SHORT_NAME}/normal_tweets'
    ...
    for date in rrule(DAILY, dtstart=LAST_WEEK_START, until=LAST_WEEK_END):
        ...
        with open(f'{TWEET_PATH}/{year}/{month}/{COIN_SHORT_NAME}_{date_str}_normal.json', 'r', encoding='utf-8-sig') as file:
            data = json.load(file)
            tweets_from_last_week.append(data.get(JSON_DICT_NAME, []))
...
def main():
    ...
    tokens = tokenize_tweets(tweets), tokens_from_last_week = tokenize_tweets(tweets_from_last_week)
